...
import { LayerGroupState, PresetColors, PresetVisibility } from "../hooks/use-layer-styles";
...
const ColorPicker = ({
  ...
  return (
    <input
      type="color"
      title={title}
      className={`cursor-pointer p-0 border-0 overflow-hidden bg-transparent transition-opacity ${
        className ? className : "w-5 h-5 rounded"
    ...
    return layerGroups
      .map((group) => {
        const matchingLayers = group.layers.filter(
          (layer) =>
            layer.id.toLowerCase().includes(lowerSearch) ||
            formatLayerName(layer.id).toLowerCase().includes(lowerSearch),
        );
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          disabled={!isReady || filteredLayerGroups.length === 0}
        >
          {filteredLayerGroups.length > 0 &&
          filteredLayerGroups.every((g) => (isSearching ? true : openGroups[g.id]))
            ? "Collapse All"
            : "Expand All"}
        </Button>
      </div>
      ...
      <div className="space-y-1.5">
        {filteredLayerGroups.map((group) => {
          ...
          const proxyStrokeColor = firstLayerId
            ? colors[firstLayerId]?.stroke || "#000000"
            : "#000000";
          ...
          return (
            <Collapsible
              ...
              className="group/collapsible border border-border/50 rounded-md overflow-hidden bg-card transition-colors"
            >
              <div className={`flex flex-col ${someVisible ? "" : "opacity-75 bg-muted/20"}`}>
                {/* Group Header */}
                <div className="flex items-center justify-between p-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <Checkbox
                      ...
                      disabled={!isReady}
                      className={`w-4 h-4 ${isChecked === "indeterminate" ? "opacity-50" : ""}`}
                    />
                    <Label
                      htmlFor={`group-visibility-${group.id}`}
                ...
                {/* Granular Layers */}
                <CollapsibleContent>
                  <div className="flex flex-col gap-1 pb-2 px-3 pt-1 border-t border-border/40 bg-muted/10">
                    {group.layers.map((layer) => {
                      const isLayerVisible = visibility[layer.id] !== false;
                      return (
                        <div
                          key={layer.id}
                          className={`flex items-center justify-between py-1.5 pl-6 pr-1 rounded hover:bg-muted/30 transition-colors ${isLayerVisible ? "" : "opacity-60"}`}
                        >
                          <div className="flex items-center gap-2 overflow-hidden">
                            ...
                            <Label
                              ...
                              className="text-[11px] truncate cursor-pointer font-normal text-muted-foreground hover:text-foreground transition-colors"
                              title={layer.id}
                            >
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <ColorPicker
                              ...
                              onChange={(e) =>
                                updateLayerColor(layer.id, layer.type, "fill", e.target.value)
                              }
                            ...
                            <ColorPicker
                              ...
                              onChange={(e) =>
                                updateLayerColor(layer.id, layer.type, "stroke", e.target.value)
                              }
                              disabled={!isReady || !isLayerVisible}
        ...
        {filteredLayerGroups.length === 0 && isReady && (
          <span className="text-sm text-muted-foreground">No layers match your search.</span>
        )}
      </div>
    </div>
  );
