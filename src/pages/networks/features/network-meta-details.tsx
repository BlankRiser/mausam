...
export const NetworkMetaDetails = () => {
  ...
  return (
    <section className='grid h-fit grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4'>
      {CardData.map((card, index) => (
        <Card
          key={card.value.toString() + index.toString()}
          className={cn(['rounded-sm bg-neutral-50 px-4 py-2 dark:bg-neutral-950', card.className])}
        >
