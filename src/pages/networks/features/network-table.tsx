...
export const NetworksTable = () => {
  ...
  return (
    <div className='overflow-auto'>
      <DataTable table={table} columns={columns} className='[&>div]:max-h-[calc(100dvh-var(--nav-height)-4rem)]' />
    </div>
  );
};
  ...
  return [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <span className='text-nowrap'>{format(new Date(row.original.PERIOD_OF_RECORD.start), 'MMM dd yyyy')}</span>
        );
      },
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <span className='text-nowrap'>{format(new Date(row.original.PERIOD_OF_RECORD.end), 'MMM dd yyyy')}</span>
        );
      },
    },
    {
