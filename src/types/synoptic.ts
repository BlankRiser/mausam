...
type Soilmoisture = {
  soil_moisture_1: Airtemp1;
  soil_moisture_2?: Airtemp1;
...
type Sonicairtempstdev = {
  sonic_air_temp_stdev_1: Airtemp1;
};

type Sonicverticalwindstdev = {
  sonic_vertical_wind_stdev_1: Airtemp1;
};

type Sonicmeridionalwindstdev = {
  sonic_meridional_wind_stdev_1: Airtemp1;
};

type Soniczonalwindstdev = {
  sonic_zonal_wind_stdev_1: Airtemp1;
};

type Sonicverticalvel = {
  sonic_vertical_vel_1: Airtemp1;
};

type Sonicairtemp = {
  sonic_air_temp_1: Airtemp1;
};
...
type Precipaccumfiveminute = {
  precip_accum_five_minute_1: Airtemp1;
};
...
type Grip1icefrictioncode = {
  grip_1_ice_friction_code_1: Airtemp1;
};

type Estimatedsnowfallrate = {
  estimated_snowfall_rate_1: Airtemp1;
};
...
type Roadsurfacecondition = {
  road_surface_condition_1?: Airtemp1;
  road_surface_condition_2?: Airtemp1;
  road_surface_condition_3?: Airtemp1;
};

type Roadfreezingtemp = {
  road_freezing_temp_1: Airtemp1;
  road_freezing_temp_2?: Dewpointtemperature1;
};

type Roadtemp = {
  road_temp_1?: Airtemp1;
  road_temp_2?: Airtemp1;
...
type Pressurechangecode = {
  pressure_change_code_1: Airtemp1;
};
...
type Visibility = {
  visibility_1: Airtemp1;
};

type Precipaccum24hour = {
  precip_accum_24_hour_1: Airtemp1;
};

type Precipaccumsixhour = {
  precip_accum_six_hour_1: Airtemp1;
};

type Cloudlayer2code = {
  cloud_layer_2_code_1: Airtemp1;
};

type Cloudlayer1code = {
  cloud_layer_1_code_1: Airtemp1;
};

type Precipaccumthreehour = {
  precip_accum_three_hour_1: Airtemp1;
};

type Precipaccumonehour = {
  precip_accum_one_hour_1: Airtemp1;
};
...
type Cloudlayer3code = {
  cloud_layer_3_code_1: Airtemp1;
};

type Weathercondcode = {
  weather_cond_code_1: Airtemp1;
};

type Sealevelpressure = {
  sea_level_pressure_1: Airtemp1;
};

type Snowdepth = {
  snow_depth_1: Airtemp1;
};
...
type Precipaccumfifteenminute = {
  precip_accum_fifteen_minute_1: Airtemp1;
};
