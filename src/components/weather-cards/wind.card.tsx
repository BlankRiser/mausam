...
export function WindCard({ data, ...rest }: WindCardProps) {
  const windSpeed = useMemo(() => {
    ...
    Object.entries(data?.STATION?.[0]?.SENSOR_VARIABLES?.wind_speed ?? {}).forEach(([key, value]) => {
      values.push({
  ...
  const windDirection = useMemo(() => {
    const values: Array<RenderedValues> = [];
    Object.entries(data?.STATION?.[0]?.SENSOR_VARIABLES?.wind_direction ?? {}).forEach(([key, value]) => {
      values.push({
        datetime: data?.STATION?.[0]?.OBSERVATIONS?.[key]?.date_time ?? 'N/A',
        value: data?.STATION?.[0]?.OBSERVATIONS?.[key]?.value ?? 'N/A',
  ...
  const windGust = useMemo(() => {
    const values: Array<RenderedValues> = [];
    Object.entries(data?.STATION?.[0]?.SENSOR_VARIABLES?.wind_gust ?? {}).forEach(([key, value]) => {
      values.push({
        datetime: data?.STATION?.[0]?.OBSERVATIONS?.[key]?.date_time ?? 'N/A',
        value: data?.STATION?.[0]?.OBSERVATIONS?.[key]?.value ?? 'N/A',
  ...
  return (
    <div className={cn([rest.className, 'data-[selected=true]:bg-red-200'])} {...rest}>
      <div className='flex w-full gap-4 rounded-xl border border-border p-2'>
        <Compass angle={Number(windDirection[0]?.value ?? 0)} />
        <div className='flex flex-col gap-2'>
          <span>
            {windDirection[0]?.value} {windDirection[0]?.valueUnit}
          </span>
          <div className='flex flex-col items-baseline-last gap-2'>
